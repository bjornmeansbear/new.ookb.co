...
$(window).load(function() {
  ...
  if (ww >= 980){
    ...
    $(window).scroll(function(){
      ...
      var ratio1 = wh/(dh-wh-288);

      $("#portrait").css("transform","translateY(" + (s-(s*ratio1)) + "px)");
      $("#intro").css("transform","translateY(" + (s*.5) + "px)");
      $("#location").css("transform","translateY(" + (s*.4) + "px)");
      $("#currently").css("transform","translateY(" + (s*.3) + "px)");
      $("#whatelse").css("transform","translateY(" + (s*.2) + "px)");
      $("#lectures").css("transform","translateY(" + (s*.1) + "px)");
